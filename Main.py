...
class Window:
    def __init__(self, master=None):
        ...
        # Texts on the screen
        self.title = Label(text="CASTELLO NOBLE", font="Lucinda 30 bold", fg="#ffffff", bg="#151515")
        self.title.place(x=0, y=40, w=self.width)
        # self.title.place(x=200, y=40)

        self.program = Label(text="Guilherme Nobre", font="Lucinda 30 bold", fg="#ffffff",
                             bg="#151515")
        self.program.place(x=0, y=380, w=self.width)
        # self.program.place(x=200, y=380)

        my_image1 = ImageTk.PhotoImage(Image.open("C:\Banco Dados/FOTOMENU.jpg"))

        image = Image.open("C:\Banco Dados/FOTOMENU.jpg")
        photo = ImageTk.PhotoImage(image)
        self.imagem = Label(text="Adicionando", image=photo, fg="#ffffff", bg="#151515")
        self.imagem.image = photo
        self.imagem.place(x=200, y=100)

        # Sidebar

        self.sidebar_position = -200
        self.sidebar = Frame(master)
        self.sidebar.configure(background="#00ff0d")
        ...
        self.btn_p1 = Button(self.sidebar, text="CORREÇÃO DE LAJE", fg="#ffffff", bg="#b40404",
                             font="System 14 bold", command=p4)
        self.btn_p1.place(x=10, y=300, w=180)

        self.btn_p2 = Button(self.sidebar, text="PILAR EXEMPLAR", fg="#ffffff", bg="#b40404",
                             font="System 14 bold", command=p3)
        self.btn_p2.place(x=10, y=250, w=180)

        self.btn_p3 = Button(self.sidebar, text="CORREÇÃO DE MOMENTOS", fg="#ffffff", bg="#b40404",
                             font="System 0 bold", command=p2)
        self.btn_p3.place(x=10, y=200, w=180)

        self.btn_p4 = Button(self.sidebar, text="CONSUMO DIÁRIO", fg="#ffffff", bg="#b40404",
                             font="System 14 bold", command=p1)
        self.btn_p4.place(x=10, y=150, w=180)

        # Fotter on sidebar
        self.footer_label = Label(self.sidebar, text="Castello Noble V1.2", font="System 14 bold",
                                  fg="#ffffff", bg="#787878")
        self.footer_label.place(x=0, y=450, w=200, h=50)

        # Button Menu
        self.btn = Button(self.main_frame, text="Menu", command=self.controll_sidebar)
        self.btn.place(x=1, y=475)

    def controll_sidebar(self):

        # A - Check if the sidebar is open or nor and changes the speed value
        ...
        # Refresing the sidebar position
        self.sidebar.place(x=self.sidebar_position, y=0, w=200, h=self.height)

        # Refresing the main frame position
